...
int main(int argc, char* argv[])
{
    ...
    c_out = fopen(strcat(argv[1], "stp"), "w");
    if (!c_out)
    {
        perror("fopen");
        return errno;
    ...
    while(1){
        ...
        if (cnt){
            ...
            /* get timestamp of the computer*/
            gettimeofday(&time_now, NULL);
            
            printf("%ld.%lds, Recv %dth msg with rate: 0x%02x | payload len: %d\n", time_now.tv_sec, time_now.tv_usec, total_msg_cnt,csi_status->rate,csi_status->payload_len);
            ...
            if (log_flag){
                ...
                fwrite(&time_now.tv_sec, 1, sizeof(time_t), c_out);
		        fwrite(&time_now.tv_usec, 1, sizeof(suseconds_t), c_out);
            }
        }
    }
    fclose(fp);
