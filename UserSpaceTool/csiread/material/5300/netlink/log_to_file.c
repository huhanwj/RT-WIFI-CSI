...
int main(int argc, char** argv)
{
	...
	c_out = open_file(strcat(argv[1], "stp"), "w");
	...
	while (1)
	{
		...
		if (count % SLOW_MSG_CNT == 0)
			printf("%ld.%lds, received %d bytes: id: %d val: %d seq: %d clen: %d\n", time_now.tv_sec, time_now.tv_usec, cmsg->len, cmsg->id.idx, cmsg->id.val, cmsg->seq, cmsg->len);
